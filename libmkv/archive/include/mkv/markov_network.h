...
namespace mkv {
    ...
    public:
        ...
        //! Retrieve the size of this network, in total number of states.
    ...
    /*! Update the Markov network "net" n times, with the input given by [f,l), and place
     output in the state vector "output".
     */
    template <typename MarkovNetwork, typename ForwardIterator, typename OutputIterator>
    void update(MarkovNetwork& net, std::size_t n, ForwardIterator f, ForwardIterator l, OutputIterator o) {
        ...
        for( ; n>0; --n) {
            net.rotate();
            std::copy(f, l, net.input_begin());

            net.top_half();
            for(typename MarkovNetwork::iterator i=net.begin(); i!=net.end(); ++i) {
                (*i)->update(net);
            }
            net.bottom_half();
        }

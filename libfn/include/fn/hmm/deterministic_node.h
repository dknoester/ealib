 ...
 */
#ifndef _FN_HMM_DETERMINISTIC_NODE_H_
#define _FN_HMM_DETERMINISTIC_NODE_H_
...
namespace fn {
	namespace hmm {
		namespace detail {
			...
			/*! deterministic HMM node.
			 */
			class deterministic_node : public hmm_node {
			public:
				...
				//! Constructor.
				deterministic_node(const genome& g, int start, int* where=0);
				
				//! Destructor.
				virtual ~deterministic_node() {
				}
				...
				//! Return a deterministic column index.
				inline int& dtable(int i) { return _ndr->entry(i,0); }
